...
public class Marin {
    ...
    public Entity planificationMarinAllerRamer(List<Entity> Entities, int nombreDeMarinsManquantsAGauche, int nombreDeMarinsManquantsADroite){
        ...
        for (int i=0;i<Entities.size();i++) {
            if (Entities.get(i) instanceof Rame) { //Pour faire ramer les marins
                ...
                if (nombreDeMarinsManquantsAGauche > 0 && Entities.get(i).getY() == 1) { // Tribord deplacementMarin est y=1
                    if (deplacementMarin < 6 && this.libre && deplacementMarin < deplacementPlusCourt) {
                        entiteRecoitMarin = i;
                        deplacementPlusCourt = deplacementMarin;
                    }
                }
                else if (nombreDeMarinsManquantsADroite> 0 && Entities.get(i).getY() == 0) { // Babord deplacementMarin'est y=0
                    if (deplacementMarin < 6 && this.libre && deplacementMarin < deplacementPlusCourt) {
                        entiteRecoitMarin = i;
                        deplacementPlusCourt = deplacementMarin;
                    }
